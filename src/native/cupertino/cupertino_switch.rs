...
use iced_native::{
    Clipboard, Color, Element, Event, event::Status, Layout, layout::{Limits, Node}, Length, mouse,
    Point, Rectangle, renderer::Style, Size, Shell, touch, Vector, Widget, window,
    widget::{tree::{State, Tag}, Tree},
};

use iced_graphics::widget::canvas::{Cache, fill::Fill, Geometry, Path};
...
pub struct CupertinoSwitch<Message> where Message: Clone {
    width:         Length,
    height:        Length,
    active_colour: Color,
    focus_colour:  Color,
...
impl<Message> Default
for CupertinoSwitch<Message>
where Message: Clone {
    fn default() -> Self {
        Self {
...
impl<Message> CupertinoSwitch<Message> where Message: Clone {
...
where B: Backend, Message: Clone {
    ...
    fn draw(
        ...
        let switch: Geometry = state.switch.draw(Size {
            ...
            if self.value {
                ...
                frame.fill(
                    &Path::circle(Point { x: width, y: height / 2.0 }, radius),
                    Fill::from(self.active_colour),
                );

                frame.fill(
                    &Path::circle(Point { x: 0.0, y: height / 2.0 }, radius),
                    Fill::from(self.active_colour),
                );
                ...
                frame.fill(
                    &Path::circle(
                        Point { x: width - padding - new_index as f32, y: height / 2.0 },
                        radius - padding
                    ),
                ...
                frame.fill(
                    &Path::circle(
                        Point { x: 0.0 + padding + new_index as f32, y: height / 2.0 },
                        radius - padding
                    ),
                    Fill::from(Color::WHITE)
                );
    ...
    fn tag(&self) -> Tag { Tag::of::<SwitchState>() }
            ...
            | Event::Touch(touch::Event::FingerPressed { .. }) => {
                ...
                if hit_x && hit_y {
                    ...
                    if self.on_changed.as_ref().is_some() {
                        shell.publish(
                            (self.on_changed.as_ref().unwrap())(!self.value)
                        );
...
impl<'a, Message, B, T> From<CupertinoSwitch<Message>>
for Element<'a, Message, Renderer<B, T>>
where B: Backend, Message: Clone + 'a {
